...
public class FakeTimerRegistry : ITimerRegistry
{
    ...
    /// </summary>
    private readonly ConcurrentDictionary<FakeTimerEntry, FakeTimerEntry> timers =
        new();

    /// <summary>
    ///     Registers a new fake timer entry and returns it.
    ///     Note how we are capturing the activation task scheduler to ensure we can tick the fake timers within the activation
    ///     context.
    /// </summary>
    public IDisposable RegisterTimer(Grain grain,
    ...
    /// </summary>
    public IEnumerable<FakeTimerEntry> GetAll()
    {
    ...
    ///     Removes a timer.
    /// </summary>
    public void Remove(FakeTimerEntry entry)
    {
        timers.TryRemove(entry, out _);
    }
}